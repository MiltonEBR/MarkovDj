function playKeyboard() {
    let pressColor = '#71db90'; //color when key is pressed
    ...
    let btnStartDj = document.getElementById('start');
    let fnPlayMarkov = (key) => {
        fnPlayKeyboard({ keyCode: key });
        setTimeout(() => {
            fnRemoveKeyBinding({ keyCode: key });
        }, 500);
    };

    let markovDj = new MarkovMusic(fnPlayMarkov, Object.keys(keyboard));
    btnStartDj.addEventListener('click', () => {
        markovDj.play();
    });

    let btnStopDj = document.getElementById('stop');
    btnStopDj.addEventListener('click', () => {
        const seq = markovDj.stop();
        showSequence(seq);

        // console.log(reverseLookup[n + ',' + i]);
    });

    let seq = document.getElementById('sequence');

    function showSequence(keyCodes) {
        seq.innerHTML = '';
        for (const key of keyCodes) {
            const keys = keyboard[key].split(',');

            const cat = keys[0].indexOf('#');

            const playedKey =
                cat >= 0
                    ? keys[0].slice(0, cat) + (__octave + parseInt(keys[1])) + '#'
                    : keys[0] + (__octave + parseInt(keys[1]));

            seq.innerHTML += playedKey + ', ';
        }

        seq.innerHTML = seq.innerHTML.substring(0, seq.innerHTML.length - 2);
    }
}
